...
%TODO Tu powinien się zacząć rodział o autonomiczności... (section) OK
...
%TODO ESP - może już nie, bo tego to całkiem dużo by było. OK
...
%TODO no może asystent brzmi ciut lepiej, ale też szału nie ma :). Niech będzie. OK
...
\end{matrix}\right.
\end{equation}
%TODO wpp - no raczej wiadomo o co chodzi. OK
...
%TODO tu brakuje \cite w txt - jest tylko pod rysunkami OK
...
W typowym użyciu CNN próbkowanie jest stosowane między innymi w celu umożliwienia znajdowania obiektów, które zajmują większy obszar obrazu. %TODO to jest niejasne - chodzi o zmianę rozdzielczości ? -no mniej więcej, chodziło mi o maxpooling z CNN.
...
W~praktyce metoda aplikowana jest do obrazu cyfrowego, więc promień jest skwantowany na kilka przedziałów długości.
...
Pierwszym krokiem algorytmu jest wybór przestrzeni barw, do której zostanie przekonwertowany obraz. 
...
Klasyfikacja znaków drogowych jest oparta o histogram gradientów (HoG) oraz maszynę wektorów wspierających (SVM). W opisywanym artykule korzystając z tej metody uzyskano skuteczność na poziomie 90\%.
%TODO Tak teraz zauważyłem, że w obu metodach klasycznych nie ma nic o rozpoznawaniu - chociaż po 2 zdania dodać. OK
...
Sieć neuronowa zbudowana jest z pięciu warstw konwolucyjnych, trzech warstw w~pełni połączonych i warstwy \textit{Softmax} (rys. \ref{fig:sign_detection3_cnn_schema}). 
...
Jest to ważne z punktu widzenia klasyfikatora SVM (maszyna wektorów wspierających), któremu dostarcza się zarówno próbek pozytywnych (obrazy ze światłami) oraz negatywnych (samo tło).
%TODO Ten SVM tak nie z gruszki, nie z pietruszki - zdanie wyjaśnienia ok
%TODO A jak tak czytam to cały akapit też niejasny - proszę to lepiej ująć. ok
%TODO Co jdst wejściem do klasyfikatora ? cechy haara \/
...
%TODO NIe bardzo to rozumiem... te kontury tylko do wizualizacji ? dopisane.
...
%TODO A podali jakoś procent skuteczności ? Tylko JBC dla ilu klas -Niestety, nie podali, w ogóle ten artykuł chyba nie był zbyt trafionym wyborem.
...
Jak wspomniano w rozdziale \ref{sec:car_general}, w celu zredukowania czasu obliczeń nie każdy piksel w wybranym obszarze jest analizowany. 
...
Obraz pozyskiwany jest z kamer umieszczonych w lusterkach. 
...
%TODO Tu jak dla mnie niejasne jest wyzaczanie tego ROI 32x32... Klasyfikator kaskadowy potrzebuje mieć na wejściu obrazy niskiej rozdzielczości - niestety nie wgłębiałem się zbyt w jego dokładne działanie.
